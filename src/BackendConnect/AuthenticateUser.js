...
export const FindUserRating = async(id)=>{
    try{
           const res= await axios.get(`${ServerIP}/user/findrating`, {params:{id:id}})
           if(res)
           return res.data
            else 
            return null
...
export const SetUserRating = async(data)=>{
    try{
        const res =  await axios.put(`${ServerIP}/user/setrating`, data)
        if(res.data)
        return res
                else
                return null
