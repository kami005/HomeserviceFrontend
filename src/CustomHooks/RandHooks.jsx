...
export const dialogReducer =(state, action)=>{
  switch(action.type){
    case 'COMPLETE':{
          return {message:'Confirm to Complete', isOpen:true, msgColor:'warning', status:'COMPLETE'}
    }
...
export const dashboardReducer = (state, action)=>{
  switch(action.type)
  {
   ...
   case 'CHANGEQOUTE':{
    let qouteAmount = action.payload.value
    let rating=action.rating
    const username=action.username, id=action.id 
    let data = [...state]
    let index =action.index
    let qouteBy =[...state[index].qouteBy]
    if(!qouteBy.length)
    {
      qouteBy=[{rating:rating, userName:username, qouteAmount:qouteAmount, startingDate:'', id:id}]
      data[index].qouteBy=qouteBy
      return data 
    }
    else{
       let indexqouteBY = qouteBy.findIndex(userName=>userName.userName === username)

      if(indexqouteBY<0)
      {
        qouteBy=[...qouteBy, {userName:username, [data.name]:data.value, id:id, rating:rating}]
      }

      else
      {

        qouteBy[indexqouteBY]={...qouteBy[indexqouteBY], qouteAmount:qouteAmount}
      }
      data[index].qouteBy=qouteBy
   ...
   case 'RATING':{
    const index = action.index, rating=action.rating
    let data = [...state]
   ...
   case 'CHANGEDATE':{
      ...
      if(!qouteBy.length)
      {
        qouteBy=[{rating:rating, userName:username, qouteAmount:'', startingDate:date}]
        data[index].qouteBy=qouteBy
        return data 
      }
      else{
        ...
        if(indexqouteBY<0)
        {
          qouteBy=[...qouteBy, {rating:rating, userName:username, startingDate:date}]
        }
  
        else
        {

          qouteBy[indexqouteBY]={...qouteBy[indexqouteBY], startingDate:date}
